...
const char* WINDOWTITLE = { "Texture Mapping -- Han Jiang" };
...
InitMenus()
{
	...
	int dismenu = glutCreateMenu(DoDistortMenu);
	glutAddMenuEntry("ON", ON);
	glutAddMenuEntry("OFF", OFF);

	int mapmenu = glutCreateMenu(DoMapMenu);
	...
	glutAddSubMenu("Distort", dismenu);
	glutAddSubMenu("Texture Map", mapmenu);
...
InitGraphics()
{
	...
	//This one is for second object if exists
	//glGenTextures(1, &Tex1);
	glBindTexture(GL_TEXTURE_2D, Tex0);// make the Tex0 texture current and set its parameters
	// If I use GL_CLAMP, there will be a black line on the sphere, GL_CLAMP_TO_EDGE can solves this. It is an advanced algorithm
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				...
				p->s = (lng * cos(Time * 1.5) + M_PI) / (2. * M_PI);
				
				printf( "p->s: %.6f\t Time change:%.6f\n", p->s, cos(Time * 1.3));
				printf("lng: %.6f\n",lng);
				printf("time: %.6f\n", Time);
				p->t = (lat + M_PI / 2.) / M_PI;
			}
