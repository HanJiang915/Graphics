...
const char* WINDOWTITLE = { "Helicopter -- Han Jiang" };
...
int		WhichPerspective;		// INSIDE or OUTSIDE
int		Xmouse, Ymouse;			// mouse values
float	Xrot, Yrot;				// rotation angles in degrees
float	Time=0;				//unit to put in Animate( )
bool		Frozen;				//debugging to freezes the animation
float	BladeAngle;			//to rotate the blades
...
Display()
{
	...
	// rotate the scene:
	//Xrot, Yrot values are set by mouse moving
	...
	//Scale is set by mouse's middle button
	...
	//reference:http://web.cse.ohio-state.edu/~machiraju.1/teaching/CSE581/Labs/walk/OPENGL_DRIVER.h
	//draw custom object:
	/*
	glBegin(GL_TRIANGLE_STRIP);
	float rate = 3.1415926f * 2 / 150;
	float radius = 10.;
	float height = 30.;
	for (int y = 0; y < 150; y++) {
		
		if (y < 30) glColor3f(0, 134 / 260., y / 60.);
		else if (y < 60) glColor3f(0, 128 / 255., (60 - y) / 60.);
		else if (y < 90) glColor3f((y - 60.) / 60., 134 / 260., 0);
		else if (y < 120) glColor3f(1., (y / 160.), 0);
		else glColor3f(134 / 260., (160 - y) / 60., 0);
		glVertex3f(cosf((y)*rate) * radius, sinf((y)*rate) * radius, 0);
		glVertex3f(cosf((y)*rate) * radius, sinf((y)*rate) * radius, height);
		glVertex3f(cosf((y + 1) * rate) * radius, sinf((y + 1) * rate) * radius, height);
		glVertex3f(cosf((y + 1) * rate) * radius, sinf((y + 1) * rate) * radius, 0);
		
	}
	glEnd();*/

	// draw the helicopter object:
	glCallList(HeliList);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., (char*)"Han Jiang-CS450");
...
InitMenus()
{
	...
	int modmenu = glutCreateMenu(DoModeMenu);
