...
GLuint	IcosList;			// object display list
...
int		Frozen;				//to freeze animation for debugging
int		Light0On;				//source light
int		Light1On;				//^
int		Light2On;				//^
float	White[] = { 1.,1.,1.,1. };
float	Time;
...
Display()
{
	...
	// draw the Icos object:
	glPushMatrix();
	glEnable(GL_LIGHTING);
	//set material and light
	//........
	//........
	SetMaterial(.5, .5, .8, 20.);
...
InitLists()
{
	...
	glColor3f(0., 0., 0.);
	glutSolidTorus(0.3, 0.7, 100, 100.);
	glEnd();
	glEndList();

	//create the Iocs:
	IcosList = glGenLists(1);
	glNewList(IcosList, GL_COMPILE);
	glShadeModel(GL_SMOOTH);
	glColor3f(-1., 0., 0.);
